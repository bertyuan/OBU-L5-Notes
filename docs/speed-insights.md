...
The instructions differ based on which framework you're using. Here are examples for popular frameworks:
...
For versions of Next.js older than 13.5, import the `<SpeedInsights>` component from `@vercel/speed-insights/react` and create a dedicated component to avoid opting out from SSR on the layout:
